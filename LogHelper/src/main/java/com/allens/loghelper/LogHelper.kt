...
object LogHelper {
    ...
    fun init(
        ...
        val handler: Handler =
            WriteHandler(context, ht.looper, path, maxLogRom * 1024 * 1000, maxLogFileSize)
        val diskLogStrategy =
    ...
    fun init(context: Context, isDebug: Boolean) {
        init(context, getBasePath(context) + "/log", 5, 6, isDebug)
    }
...
class WriteHandler(
    ...
    override fun handleMessage(msg: Message) {
        ...
        val logFile: File = getLogFile(path, maxLogRom, maxLogFileSize, context = context)
        try {
...
private fun getLogFile(path: String, maxLogRom: Int, maxLogFileSize: Int, context: Context): File {
    val folder = File(path)
    if (!folder.exists()) {
        folder.mkdirs()
    }

    var newFileCount: Int = ShareLogUtil.getInstance(context).getInt("log_index", 1)
    ...
    if (existingFile != null) {
        if (existingFile.length() >= maxLogRom) {
            ShareLogUtil.getInstance(context).putInt("log_index", newFileCount)
